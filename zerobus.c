...
FILE* FileptrLoadFlowResults(FILE* fpo5, Rect* Volt, Rect* Sinj, BusData* bus, LineData* line, Rect** S, Rect* LineLoss, Polar* V, int nb, int nl) {
  int i, j, busno;
...
int main(int argc, char* argv[]) {
  ...
  printf("\n\t\tTest system considered is %d bus distribution system\n", sys.nbus);
  printf("\nEnter 1 for base case \nEnter bus number where all quantities are unknown : \t");
  scanf("%d", &ZeroIndex);
    
  if( (ZeroIndex > sys.nbus) || (ZeroIndex < 1) ) {
  ...
  fpo5 = FileptrLoadFlowResults(fpo5, Volt, Sinj, bus, line, S, LineLoss, V, sys.nbus, sys.nline); 
